...
\begin{frame}
    \begin{figure}[h]
        ...
        \begin{tikzpicture}
            ...
            \draw [line width = 2pt, ->] 
                (\xA,\yStart+\dY*2) 
                node [label={[label distance=\lD]left:{$t_A$}}]{}
                -- (\xB,\yStart+\dY*2) 
                node [label={[label distance=\lD]right:{$t_B$}}]{};
            % Draw light backward
            \draw [line width = 2pt, ->] 
                (\xB,\yStart+\dY*3) 
                node [label={[label distance=\lD]right:{}}]{}
                -- (\xA,\yStart+\dY*3) 
                node [label={[label distance=\lD]left:{$t^{\prime}_A$}}]{};
        \end{tikzpicture}
    \end{figure}        
\end{frame}
